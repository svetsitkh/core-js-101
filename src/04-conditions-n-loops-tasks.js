...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return (x1 > rect2.left) && (y1 > rect2.top);
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr.filter((el) => el === arr[i]).length === 1) return arr[i];
  }
  return null;
}
...
function getDigitalRoot(num) {
  let result = num.toString().split('').reduce((n, l) => n + Number(l), 0);
  if (result > 9) {
...
function getCommonDirectoryPath(pathes) {
  const firstPathPart = pathes[0].split('/');
  let result = '';
  for (let i = 0; i < firstPathPart.length; i += 1) {
    const directory = firstPathPart[i];
    const isCommon = pathes.every((path) => path.split('/')[i] === directory);
    if (isCommon) {
      result += (`${directory}/`);
    } else {
      break;
